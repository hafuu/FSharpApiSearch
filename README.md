...
| 型パラメーター               | `List<T> -> int`<br>`<TKey, TValue>` : Dictionary<TKey, TValue>` |
...
### 型パラメーター
1文字の型名は型パラメーターとして扱われます。

2文字以上の型パラメーターを扱いたい場合は`<t> : signature`と書きます。
例えば、`<TKey, TValue> : Dictionary<TKey, TValue>`というクエリは`Tkey`と`TValue`を型パラメーターとして扱います。
名前検索と組み合わせる場合は`name<t> : signature`と書きます。
...
### フレキシブル型
フレキシブル型とは、指定した基本型またはインターフェイスと互換性のある型を指定する制約です。
フレキシブル型の挙動は、型パラメーター制約の`where T : <基本クラス名 or インターフェイス名>`と似ています。

クエリでフレキシブル型を使用するには`#type`と書きます。`type`の部分には型名とインターフェイス名を指定できます。型パラメータ、ワイルドカードは指定できません。
