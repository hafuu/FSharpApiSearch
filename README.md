...
.NET形式、もしくはタプル形式の他引数メソッドを検索するためには、`arg`部分にタプル形式 (`receiver => arg1 * arg2 -> returnType`) と書きます。
カリー化形式 (`receiver => arg1 -> arg2 -> returnType`) で書いた場合、.NET形式のメソッドはマッチしないので注意してください。
...
#### 静的メンバ
モジュール内の値と同じクエリで検索できます。インスタンスメソッドと同じ様にタプル形式とカリー化形式の区別に注意してください。
