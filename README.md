...
### 型パラメーター
クエリ中に型パラメーターを使用するには`<t> : signature`と書きます。
名前検索と組み合わせる場合は`name<t> : signature`と書きます。

クエリの型パラメーター名は検索対象の型パラメーター名と一致している必要はありません。
例えば、`<a> : List<a>`というクエリは`System.Collections.Generics.List<T>`型とマッチします。
...
クエリでフレキシブル型を使用するには`#type`と書きます。`type`の部分には型名とインターフェイス名を指定できます。ジェネリックパラメータ、ワイルドカードは指定できません。

例えば、`<T> : #IEnumerable<T>`は`List<T>`、`IList<T>`、`T[]`などにマッチします。
